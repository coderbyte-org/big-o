...
class GenericCalculator implements Calculator {
    ...
    calculate(runTimes: RunTime[], speed: AlgorithmSpeed): BigO {
        let nGroupedRunTimes = this.groupRunTimes(runTimes);
        switch (true) {
    ...
    protected groupRunTimes(runTimes: RunTime[]): NGrouped {
        let tempNGrouped: { [key: number]: number[] } = {};
        let keys: number[] = [];
        let nGroupedRunTimes: NGrouped = {};
        for (let i = 0; i < runTimes.length; i++) {
            let sampleSize: number = runTimes[i].n;
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            let sum: number = tempNGrouped[keys[i]].reduce(
