...
function convertString(argument: string): string {
    return '"' + argument + '"';
}

function convertArray(argument: Array<number|string>, language: Language): string {
    const arrayMemberType = typeof argument[0];
    if (arrayMemberType === 'string') {
        argument = argument.map(value => convertString(value as string));
    }
